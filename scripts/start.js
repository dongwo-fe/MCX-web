...
const { PROXY_LIST } = require('./config');
...
inquirer
    .prompt({
        ...
        message: '请选择一个要代理的环境',
        default: '0',
        choices: () => PROXY_LIST.map((item, index) => index + '|' + item.name),
        filter: (data) => data.split('|')[0] * 1,
    })
    .then((res) => {
        fs.writeFileSync(path.join(__dirname, '../.dev.proxy.json'), JSON.stringify({ proxy: PROXY_LIST[res.name].proxy }));
        process.env.SERVE_ENV = PROXY_LIST[res.name].SERVE_ENV;
        upDevServer();
    });
function upDevServer() {
