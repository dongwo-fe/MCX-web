...
const uploadCanary = require('./uploadCanary');
const buildData = require('../config/builddata');
...
const { SERVE_ENV = 'default' } = process.env;

// auto-config的代理
const proxy_list = {
    sit: 'https://acsit.jrdaimao.com',
    uat: 'https://acsit.jrdaimao.com',
    dev: 'https://acsit.jrdaimao.com',
    dev1: 'https://acsit.jrdaimao.com',
    dev2: 'https://acsit.jrdaimao.com',
    dev3: 'https://acsit.jrdaimao.com',
    dev4: 'https://acsit.jrdaimao.com',
    gray: 'https://ac.jrdaimao.com',
...
const topic_list = {
    ...
    dev1: 'https://topicdev1.jrdaimao.com',
...
if (SERVE_ENV !== 'default') {
    const file_path = path.join(__dirname, '../docker/nginx/conf.d');
    const template = fs.readFileSync(file_path + '/default.temp', {
        encoding: 'utf-8',
    });
    //替换热配
    let conf = template.replace(/{{ip}}/g, proxy_list[SERVE_ENV]);
    conf = conf.replace(/{{outPath}}/g, buildData.outPath);
    console.log('config代理地址', proxy_list[SERVE_ENV]);
    //替换topic
    conf = conf.replace(/{{topic}}/g, topic_list[SERVE_ENV]);
    console.log('topic代理地址', topic_list[SERVE_ENV]);
    //替换大数据
    conf = conf.replace(/{{bigdata}}/g, 'https://bigdata.jrdaimao.com');
    console.log('topic代理地址', 'https://bigdata.jrdaimao.com');
...
checkBrowsers(paths.appPath, isInteractive)
    ...
    .then((previousFileSizes) => {
        ...
        fs.emptyDirSync(paths.appBuildRoot);
